...
const LoginPage = () => {
  ...
  const getRequestError = (err) => {
    ...
    if (
      ...
      return "Authentication service is unavailable. Start the backend on port 5000 and try again.";
    }

    if (!err.response) {
      return "Authentication service is unavailable. Check that the backend is running on port 5000.";
    }

    return err.message ? `Error: ${err.message}` : "An unknown error occurred. Please try again.";
  };
  ...
  const logInUser = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(apiUrl(ENDPOINTS.auth.login), { email, password });

      if (response.data?.access_token) {
        const authenticatedUser = await fetchCurrentUser(response.data.access_token);
        storeAuthSession({
      ...
      <div className="auth-shell">
        <div className="auth-container">
          ...
          <section className="auth-intro">
            ...
            <h1 className="auth-title">Sign in to continue your diagnostic workflow</h1>
            <p className="auth-subtitle">
              Access your patient review tools, MRI screening workspace, and reporting environment from one secure clinical login.
            </p>
            <div className="auth-meta">
              <div>
                <span className="auth-meta-label">Environment</span>
                <span className="auth-meta-value">Protected session</span>
              </div>
              <div>
                <span className="auth-meta-label">Access</span>
                <span className="auth-meta-value">Clinician portal</span>
              </div>
              <div>
                <span className="auth-meta-label">Security</span>
                <span className="auth-meta-value">Encrypted sign-in</span>
              </div>
            </div>
          </section>
          ...
          <section className="auth-card">
            <div className="auth-card-header">
              ...
              <p className="auth-card-copy">Enter your credentials below to open the BTDS workspace.</p>
            </div>
            ...
            <form onSubmit={logInUser} noValidate className="auth-form">
              <div>
              ...
              <div className="auth-row">
                ...
                <span className="auth-note">Password reset unavailable</span>
              </div>
